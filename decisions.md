...
**Follow-up UX gap, also caught only by looking at it live:** a plain warning icon gives no signal that it's clickable — it reads as a passive "something's wrong here" indicator, not a control. Fixed by (1) appending an explicit action hint to the tooltip itself ("Click to confirm or edit", or "Click to confirm" where the field isn't editable, e.g. line items) below the reason, separated by a thin divider, and (2) giving the icon a stronger hover state (brighter border/background) so it reads as pressable rather than decorative. The tooltip now doubles as both the explanation and the affordance, since a separate visual cue (e.g. a chevron) would add more clutter for the same information the tooltip already has room for.
